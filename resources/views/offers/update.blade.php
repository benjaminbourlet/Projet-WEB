                ...
                <div class="mb-4">
                    ...
                    <select id="skills" name="skills[]" multiple
                        ...
                        @foreach($skills as $skill)
                            <option value="{{ $skill->id }}" {{ in_array($skill->id, $offer->skills->pluck('id')->toArray()) ? 'selected' : '' }}>
                                {{ $skill->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select id="departments" name="departments[]" multiple
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ in_array($department->id, $offer->departments->pluck('id')->toArray()) ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
